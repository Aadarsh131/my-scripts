...
def reduce_pdf_size(input_pdf, output_pdf, image_quality=18, dpi=165, crop_coords_list=None, enable_cropping=True):
    ...
    print(f"Reduced PDF saved as: {output_pdf}")

if __name__ == "__main__":
    # Example usage
    input_pdf = "/home/aadarsh/Downloads/1REducePDFSizeJiju/experience letter.pdf"
    output_pdf = "/home/aadarsh/Downloads/1REducePDFSizeJiju/experience_letter_compressed.pdf"
    crop_coords_file = "crop_coords.json"
    enable_cropping = False  # Toggle this value to enable/disable cropping

    if not os.path.exists(input_pdf):
        print(f"Input file {input_pdf} not found.")
    else:
        # Load existing crop coordinates if available
        crop_coords_list = load_crop_coords(crop_coords_file)

        if enable_cropping and not crop_coords_list:
            # Interactive cropping example
            crop_coords_list = []
            temp_image_folder = "temp_images"
            os.makedirs(temp_image_folder, exist_ok=True)

            # Extract pages as images for cropping
            pdf_document = fitz.open(input_pdf)
            for page_num in range(len(pdf_document)):
                page = pdf_document.load_page(page_num)
        ...
        reduce_pdf_size(input_pdf, output_pdf, crop_coords_list=crop_coords_list, enable_cropping=enable_cropping)
